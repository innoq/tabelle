...
import { addRadioFocus, transformHeaders } from "../header/header";
...
export default class Tabelle extends HTMLElement {
	...
	setFocus () {
		addRadioFocus(find(this, "thead th"));

		if (this.focused) {
			let toFocus;
			let { name, value } = this.focused;
			if (name === "sort") {
				toFocus = this.querySelector(`.tabelle-arrow[value="${value}"]`);
			} else {
				toFocus = this.querySelector(`.tabelle-input[name="${name}"]`);
			}
	...
	submitForm () {
		submit(this.form).then(response => {
			...
			return response.text().then(html => ({ html, uri: response.url }));
		}).then(({ html, uri }) => {

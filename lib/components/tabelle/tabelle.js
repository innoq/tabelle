/* eslint-env browser */
import { listenFor, template2dom, extractContent, submitForm as submit, listenForChange } from '../../util'
...
function createHeader ({ form, name, input, value, nosort, nofilter, currentSort }, tdContent) {
  ...
  const idUp = idGen()
  const upRadio = nosort ? '' : arrowRadio(form, idUp, name, 'asc', currentSort === name + '-asc')
  const upLabel = nosort ? '' : arrowLabel(idUp, columnName, 'asc')

  const idDown = idGen()
  const downRadio = nosort ? '' : arrowRadio(form, idDown, name, 'desc', currentSort === name + '-desc')
  const downLabel = nosort ? '' : arrowLabel(idDown, columnName, 'desc')

  const filter = setFormFor(input, form) || createFilter(form, name, value, columnName, nofilter)
...
function transformTabelle (tabelle) {
  ...
  transformHeaders(find(tabelle, 'thead th'), tabelleId(tabelle), tabelle.getAttribute('sort') || '')
  return tabelle
}
...
export default class Tabelle extends HTMLElement {
  ...
  addListeners () {
    ...
    const valueCache = {}
    this.addEventListener('change', listenForChange('tabelle-input', submitForm, valueCache))
    this.addEventListener('change', listenForChange('tabelle-arrow', submitForm, valueCache))
    this.addEventListener('keyup', debounce(this.debounce, listenForChange('tabelle-input', submitForm, valueCache)))

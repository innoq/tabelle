		...
		this.list.items.forEach(item => {
				...
				for (let key in item.values()) {
					...
					item.found = item.found ||
						value.toLowerCase().trim().
							includes(this.globalFilter.toLowerCase().trim());
				}
			}
			...
			for (let key in this.valueCache) {
				...
				let filterString = this.valueCache[key] &&
					normalizeString(this.valueCache[key]);

				item.found = item.found &&
					normalizeString(value).includes(filterString);
			}
		});
	}
		...
		mutations.forEach(mutation => {
			mutation.addedNodes.forEach(node => {
				if (node.tagName && node.tagName.toLowerCase() === "tr") {
					...
					// Skip updates originating inside embeddable subtrees to avoid
					// reindexing when transcluded content updates its internal DOM.
					const inEmbeddable =
						(mutation.target &&
							mutation.target.closest &&
							mutation.target.closest("[data-embeddable]")) || false;
...
function normalizeString(string) {
	let normalizedWhitespace = string.trim().replaceAll(/\s+/g, " ");

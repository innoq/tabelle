...
export class TabelleCljs extends HTMLElement {
	...
	filter() {
		this.list.search(this.globalFilter || "", this.searchFunction.bind(this));
	}

	searchFunction(searchString, _columns) {
		this.list.items.forEach(item => {
			item.found = searchString === "";

			for (let key in item.values()) {
				item.found = item.found || item.values()[`${key}`].filter.toLowerCase().trim().includes(searchString);
			}

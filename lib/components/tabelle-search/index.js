...
export class TabelleSearch extends HTMLElement {
	connectedCallback() {
		...
		this.input.addEventListener("keyup", debounce(this.debounce, this.sendSearchEvent.bind(this)));
		this.input.addEventListener("search", this.sendSearchEvent.bind(this));
		this.sendSearchEvent();

		find(this, "[type=submit], button:not([type=button]):not([type=reset])")
	...
	sendSearchEvent() {
		...
		eventTarget.dispatchEvent(new CustomEvent("tabelle-search", { detail: {

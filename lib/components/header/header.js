...
function findFilter (th, formId) {
	let input = th.querySelector(".tabelle-input");
	if (input && formId) {
		input.setAttribute("form", formId);
	}

	/* use the direct child of the `th` containing the .tabelle-input as the
	filter field */
	return input && input.closest("th > *");
}

function createHeader ({ form, name, filter, value, nosort, nofilter, currentSort },
		tdContent) {
...
export function transformHeaders (headers, formId, currentSort, nofilter) {
	let newHeaders = headers.filter(th => th.getAttribute("name") && !th.querySelector(".tabelle-header")).map(th => {
		...
		th.setAttribute("data-name", name);
		th.setAttribute("scope", "col");
		th.innerHTML = newContent.outerHTML;
...
*/
export function addRadioFocus(headers) {
